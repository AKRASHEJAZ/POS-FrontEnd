...
class ReturnDetailDialog extends StatelessWidget {
  ...
  const ReturnDetailDialog({super.key, required this.returnRecord});

  static Future<void> show(BuildContext context, ReturnModel returnRecord) {
    return showDialog<void>(
      context: context,
      builder: (_) => ReturnDetailDialog(returnRecord: returnRecord),
    );
  }
  ...
  Widget build(BuildContext context) {
    final rows = returnRecord.actions.map((action) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _kv(context, 'Sale ID', '${returnRecord.saleId}'),
              _kv(context, 'Customer ID', '${returnRecord.customerId}'),
              _kv(context, 'Date', _date(returnRecord.returnDate)),
              _kv(
                context,
                'Total qty',
                _formatQuantity(returnRecord.totalReturnedQuantity),
              ),
              _kv(context, 'Reason', returnRecord.reason ?? '-'),
              const SizedBox(height: 12),
              Text(
                'Returned items',
